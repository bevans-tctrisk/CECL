...
def _inject_prior_acl(workspace_root: str, short: str, period: str,
    ...
    if not read["ok"] and not read.get("values"):
        # The prior workbook has formulas but no cached results -- it
        # was generated by openpyxl and never opened in Excel. Force a
        # recalc-and-save via COM so the cached values appear, then
        # retry the read. This auto-heals old reports written before
        # the post-run recalc was wired in.
        recalc = excel_recalc.recalc_and_save(prior_path)
        result["prior_recalc"] = recalc
        if recalc.get("ok"):
            read = runs_service.read_prior_acl_values(prior_path)
    if not read["ok"]:
...
def _recalc_outputs(output_files: list[dict]) -> list[dict]:
    """Best-effort post-run Excel recalc on each output workbook so
    cached formula values are persisted to disk. Future-quarter carry
    reads (``runs_service.read_prior_acl_values``) depend on these
    cached values; without this step, every fresh report would require
    the analyst to manually open + save in Excel before the next
    quarter's Prior ACL block resolves correctly.

    Returns one result dict per file. Never raises.
    """
    results: list[dict] = []
    for entry in output_files or []:
        path = entry.get("path") or ""
        if not path:
            continue
        results.append(excel_recalc.recalc_and_save(path))
    return results
